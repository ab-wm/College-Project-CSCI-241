...
def selection_sort(arr):
  for k in range(len(arr)):
    minloc = k # Stores index of minimum value
    j = k + 1
    while j < len(arr):
      if arr[j] < arr[minloc]: # If smaller value found, store index
        minloc = j
      j = j + 1
    arrswap = arr[k] # Values are swapped using the arrswap variable
    arr[k] = arr[minloc]
    arr[minloc] = arrswap
